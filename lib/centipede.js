import { centipedeParams } from "@/lib/params/centipede_move";
...
export function drawCentipede(ctx, state, timeMs, useSprites, getSprite) {
  ...
  ctx.lineWidth = 1.0;
  const { ribStep, ribLenHead, ribLenTail, sizeBlend, strideMs, bodySpriteScale, legSpriteScale } = centipedeParams;
  ...
  for (let i = 2; i < n - 1; i += ribStep) {
    ...
    if (bodyImg) {
      ...
      drawImageCenteredRotated(ctx, bodyImg, cx, cy, angle, bodySpriteScale);
    } else {
      ctx.save();
    ...
    for (const side of [-1, 1]) {
      ...
      if (legImg) {
        ...
        drawImageCenteredRotated(ctx, legImg, mx, my, ang, legSpriteScale);
      } else {
        ctx.beginPath();
        ctx.moveTo(cx, cy);
