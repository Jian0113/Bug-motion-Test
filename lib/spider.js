...
export function drawSpider(ctx, state, timeMs, getSprite) {
  ...
  const jawScaleCfg = scaleClamp(config.jawScale, 1.2);
  const legScaleCfg = scaleClamp(config.legScale, 1.6);
  ...
  const legRots = config.legRots || { 1: 0, 2: 0, 3: 0, 4: 0 };
  ...
  fetch('http://127.0.0.1:7242/ingest/aa0df7a3-9505-41db-a875-29a987833b4d',{
    ...
    body:JSON.stringify({
      ...
      runId:'pre-fix',
      hypothesisId:'H1',
      location:'lib/spider.js:config',
      message:'spider config',
      data:{headScaleCfg, bodyScaleCfg, jawScaleCfg, legScaleCfg, headGapCfg, bodyGapCfg, jawLenCfg, legAttachCfg, jawSpreadDegCfg, jawAnchorGapCfg, jawAnchorHeadGapCfg},
      timestamp: Date.now(),
    })
  }).catch(()=>{});
  // #endregion
  ...
  // #region agent log
  if (!drawSpider._lastLog) drawSpider._lastLog = 0;
  if ((timeMs || 0) - drawSpider._lastLog > 1200) {
    drawSpider._lastLog = (timeMs || 0);
    fetch('http://127.0.0.1:7242/ingest/aa0df7a3-9505-41db-a875-29a987833b4d',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({
        ...
        runId:'pre-fix',
        hypothesisId:'H4',
        location:'lib/spider.js:legs',
        message:'leg sprites & anchors',
        data:{
          hasSprites: legSprites.map(Boolean),
          attachRadius: rBody * legAttachCfg,
  ...
  for (let i = 0; i < anchorAngles.length; i++) {
    ...
    // #region agent log
    if (!drawSpider._lastLegLog) drawSpider._lastLegLog = 0;
    if ((timeMs || 0) - drawSpider._lastLegLog > 1400) {
      drawSpider._lastLegLog = (timeMs || 0);
      fetch('http://127.0.0.1:7242/ingest/aa0df7a3-9505-41db-a875-29a987833b4d',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({
          ...
          runId:'pre-fix',
          hypothesisId:'H5',
          location:'lib/spider.js:legSprite',
          message:'leg sprite render',
          data:{
            idx:i,
            pair,
            side,
    ...
    // #endregion
    if (legSprite) {
      drawImageCenteredRotated(ctx, legSprite, axl, ayl, baseTheta + (legRotDeg * Math.PI) / 180, legScaleCfg);
      continue;
    }
